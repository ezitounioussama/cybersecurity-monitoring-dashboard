...
import { SeverityBadge, CriticalityBadge } from "@/components/shared/severity-badge";
import {
  AlertStatusBadge,
  AssetStatusBadge,
  PatchStatusBadge,
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        <Button variant="ghost" size="sm" asChild className="-ml-2 h-8 text-muted-foreground">
          <Link href="/assets"><IconArrowLeft className="size-4" /> Back to assets</Link>
        </Button>
        ...
        <p className="font-mono text-sm text-muted-foreground">{asset.ipAddress}</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
              ...
              asset.vulnerabilities.map((v) => (
                <div key={v.id} className="flex items-center justify-between gap-2 rounded-lg border p-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">CVSS {v.cvssScore.toFixed(1)}</p>
                  </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Recent alerts ({asset.alerts.length})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
              ...
              asset.alerts.map((a) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{formatRelative(a.detectedAt)}</p>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
