...
export const GET = apiRoute("read", async (_req, { ctx }, route) => {
  const { id } = await route.params;
  const alert = await alertService.getById(ctx, id);
  return ok(alert);
});

export const PATCH = apiRoute("mutation", async (req, { ctx }, route) => {
  const { id } = await route.params;
...
export const DELETE = apiRoute("mutation", async (_req, { ctx }, route) => {
  const { id } = await route.params;
